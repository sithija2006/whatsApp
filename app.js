...
function toggleAuthMode() {
  ...
  if (isSignupMode) {
    loginTitle.textContent = "Join ChatFlow"
    loginSubtitle.textContent = "Create your account to get started"
    authButtonText.textContent = "Create Account"
    switchAuth.innerHTML = "Already have an account? <span>Sign in</span>"
  } else {
    loginTitle.textContent = "Welcome to ChatFlow"
    loginSubtitle.textContent = "Connect with friends instantly"
    authButtonText.textContent = "Sign In"
    switchAuth.innerHTML = "Don't have an account? <span>Sign up</span>"
  }
  hideError()
}
      ...
      (user) => `
        <div class="user-item ${selectedUser?.uid === user.uid ? "active" : ""}" 
            ...
            <div class="user-item-info">
                ...
                <p>${user.online ? "Active now" : formatLastSeen(user.lastSeen)}</p>
            </div>
        </div>
    `,
    )
...
function selectUser(user) {
  ...
  chatUserStatus.textContent = user.online ? "Active now" : formatLastSeen(user.lastSeen)
...
// Improve message input focus on mobile
...
messagesContainer.addEventListener("scroll", requestScrollUpdate, { passive: true })
...
document.querySelectorAll(".auth-button, .send-button, .user-item").forEach(addTouchFeedback)

// Handle network status for mobile
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    ...
    // This is just a placeholder - you'd need to create a service worker file
    console.log("Service Worker support detected")
  })
}

// Handle mobile-specific message sending
messageForm.addEventListener("submit", (e) => {
  e.preventDefault()
  const text = messageInput.value.trim()
  if (text) {
